...
const Dashboard = ({ isSidebarOpen, selectedAnalysis }) => {
  ...
  /* useEffect(() => {
    if (selectedAnalysis) {
      ...
      setTitle(selectedAnalysis.title || "Untitled");
      setSummary(selectedAnalysis.summary || "");
      setKeywords(Array.isArray(selectedAnalysis.keywords) ? selectedAnalysis.keywords : []);
      setTablesData(
        selectedAnalysis.tables?.map((table) => ({
          ...table,
          pie_url: null,
          bar_url: null,
          pie_error: table.pie_error || null,
          bar_error: table.bar_error || null,
        })) || []
      );
      setCitations(selectedAnalysis.citations || []);
      setRecommendations(selectedAnalysis.recommendations || []);
      setVisualContent(selectedAnalysis.images || []);
      setAuthors(selectedAnalysis.authors || []);
      setChatHistory(selectedAnalysis.chat || []);
      setError("");
      setLoading(false);
      setUploadedFile(null);
      console.log("Tables loaded:", selectedAnalysis.tables);
    }
  }, [selectedAnalysis]); */
  ...
  if (selectedAnalysis) {
    ...
    setSummary(selectedAnalysis.summary || "");
    setKeywords(Array.isArray(selectedAnalysis.keywords) ? selectedAnalysis.keywords : []);
    const tables = selectedAnalysis.tables?.map((table) => ({
      ...table,
      pie_url: table.pie_url || null,
      bar_url: table.bar_url || null,
      pie_error: table.pie_error || "Pie chart not available",
      bar_error: table.bar_error || "Bar chart not available",
    })) || [];
    ...
    // Optionally re-fetch visualizations if missing
    if (tables.some((table) => !table.pie_url && !table.bar_url && !table.pie_error && !table.bar_error)) {
      console.warn("Missing visualizations for some tables, consider re-processing PDF");
      // Add logic to re-fetch or re-process if needed
    }
  }
}, [selectedAnalysis]);
  ...
  const fetchExtractedVisualContent = async (file) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(
          `Failed to extract visual content: ${response.status} - ${errorText}`
        );
      }
      const data = await response.json();
      if (data.error) throw new Error(data.error);
      const allVisualContent = (data.visuals || []).map((item) => ({
        name: item.name,
        image: item.image,
      }));
  ...
  const fetchAuthors = async (file) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(
          `Failed to extract authors: ${response.status} - ${errorText}`
        );
      }
      const data = await response.json();
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const summaryResponse = await fetch(
        `${API_BASE_URL}/api/summarize-pdf`,
        {
          method: "POST",
          body: summaryFormData,
          credentials: "include",
      ...
      if (!keywordResponse.ok)
        throw new Error(`Failed to fetch keywords: ${keywordResponse.status}`);
      const keywordData = await keywordResponse.json();
      setKeywords(Array.isArray(keywordData.keywords) ? keywordData.keywords : []);
      console.log("Keywords set to:", keywordData.keywords);
      ...
      if (tableResponse.ok) {
        ...
        if (tableData.tables) {
          console.log(`Found ${tableData.tables.length} tables`);
          tables = tableData.tables.map((table, idx) => {
            const graphEntry = tableData.graphs.find(
              (g) => g.table_index === table.index
            ) || {};
            return {
              ...table,
              pie_url: graphEntry.pie_url || null,
              bar_url: graphEntry.bar_url || null,
              pie_error: graphEntry.pie_error || null,
              bar_error: graphEntry.bar_error || null,
            };
          });
          setTablesData(tables);
        }
        if (tableData.message) {
          console.log("Table extract message:", tableData.message);
        }
      } else {
        throw new Error(`Failed to fetch tables: ${tableResponse.status}`);
      }
      ...
      const extractedTitle =
        titleData.title &&
        titleData.title !== "Title not found" &&
        titleData.title.trim()
          ? titleData.title
          : "Untitled";
      setTitle(extractedTitle);
      ...
      const citationResponse = await fetch(
        `${API_BASE_URL}/api/extract-citations`,
        {
          method: "POST",
          body: citationFormData,
          credentials: "include",
  ...
  const handleChat = async () => {
    ...
    try {
      ...
      if (!queryResponse.ok) {
        const errorData = await queryResponse.json();
        throw new Error(
          `Failed to fetch query response: ${queryResponse.status} - ${
      ...
      const updatedChat = [
        ...chatHistory,
        { message: chatMessage, response: responseText },
      ];
      const currentAnalysis = selectedAnalysis || userData?.recent_analysis || {};
      const analysisId = currentAnalysis._id || Date.now().toString();
      ...
      const analysisData = {
        pdf_id: analysisId,
        filename:
          uploadedFile?.name || currentAnalysis.filename || "Recent PDF",
        title: title || currentAnalysis.title || "Untitled",
        summary: summary || currentAnalysis.summary || "",
        keywords: keywords.length ? keywords : currentAnalysis.keywords || [],
        tables: tablesData.length ? tablesData : currentAnalysis.tables || [],
        citations: citations.length ? citations : currentAnalysis.citations || [],
        images: visualContent.length
          ? visualContent
          : currentAnalysis.images || [],
        authors: authors.length ? authors : currentAnalysis.authors || [],
      ...
      if (!saveResponse.ok) {
        const errorData = await saveResponse.json();
        throw new Error(
          `Failed to save chat analysis: ${saveResponse.status} - ${
            errorData.error || "Unknown error"
          }`
        );
      }
      ...
      window.dispatchEvent(
        new CustomEvent("analysisUpdated", { detail: { analysis_id: analysisId } })
      );
    } catch (error) {
      console.error("Error in handleChat:", error);
      setError(error.message || "Failed to process chat query.");
  ...
  return (
    <DashboardContainer $isSidebarOpen={isSidebarOpen}>
      {console.log("Rendering Dashboard with keywords:", keywords)}
      {!hasResults && !loading && (
        <>
          <UploadButton setUploadedFile={setUploadedFile} handleUpload={handleUpload} />
          <Features />
      ...
      {hasResults && !loading && !error && (
        <>
          ...
          {tablesData.length > 0 && (
            <div>
              {tablesData.reduce((rows, table, index) => {
                if (index % 2 === 0) {
                  rows.push([table]);
                } else {
                ...
                <FlexRow key={`row-${rowIndex}`}>
                  {row.map((table, colIndex) => (
                    <Column key={`table-${table.index}-${colIndex}`}>
                      <Graph table={table} />
                    </Column>
                  ))}
                </FlexRow>
